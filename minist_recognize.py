...
#载入minist数据集，如果指定地址/path/to/MNIST_data,当前无数据时会自动下载
...
# #MNIST数据集相关常数
# INPUT_NODE=784#输入层节点数，对于MINIST数据集就是图片像素，数组纬度
# OUTPUT_NODE=10#输出层节点数，等于类别数，0～9这10个数字
#
# #配置神经网络参数
# LAYER1_NODE=500 #只用一个隐藏层且含有500个节点
# BATCH_SIZE=100 #batch中数据个数
#
# LEARN_RATE_BASE=0.8#基础学习率
# LEARN_RATE_DECAY=0.99#学习率衰减
# REGULARIZATION_RATE=0.0001#描述模型复杂度的正则项损失函数的中的系数
...
# 配置神经网络的参数
LAYER1_NODE = 1000  # 隐藏层节点数。这里使用只有一个隐藏层的网络结构作为样例。这个隐藏层有500个节点
BATCH_SIZE = 2  # 一个训练 batch 中的训练数据个数。数字越小时，训练过程越接近随机梯度下降；数字越大时，训练越接近梯度下降
...
def train(mnist):
    ...
    # 生成隐藏层的参数，tf.truncated_normal截尾正态分布,return A tensor of the specified shape filled with random truncated normal values.
    weights1 = tf.Variable(tf.truncated_normal([INPUT_NODE, LAYER1_NODE], stddev=0.1))#<tf.Variable 'Variable:0' shape=(784, 500) dtype=float32_ref>
    ...
    y = inference(x, None, weights1, biases1, weights2, biases2)#weights1加激活函数，weights2未加，最用softmax用weights2
    ...
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=y, labels=tf.argmax(y_, 1))#0是列对应，1是行对应，分别取一行或者一列最大值对应的下标
    ...
    with tf.Session() as sess:
        ...
        # print('y_.shape', mnist.test.labels.shape, np.array(mnist.test.labels),sess.run(tf.argmax(mnist.test.labels,1)))#y_.shape (10000, 10) [[0. 0. 0. ... 1. 0. 0.],[7 2 1 ... 4 5 6],(10000,)
        # 迭代地训练神经网络
        ...
        for i in range(TRAINING_STEPS):
            ...
            _,yy,average_yy=sess.run([train_op,y,average_y], feed_dict={x: xs, y_: ys})
            # print('xs.shape',xs.shape)
            # print('all_variables',tf.all_variables())
            # print('trainable_variables', tf.trainable_variables())
            # break
            print('yy:',yy,'\n','average_yy',average_yy)
            print('gloabl_step',sess.run(global_step))
            '''gloabl_step 2108
                gloabl_step 2109
                gloabl_step 2110
                gloabl_step 2111'''
